...
- **Before tagging a release, run the full-circle integration gate on every product that consumes this package.** Running a consumer's gate is validation, not a push to its repository. An assertion that **regressed** since the previous tag blocks the tag; an assertion already failing before the change does not, and is recorded. A gate that could not run is not a passing gate. A change to wire behavior is not validated by this repository's own tests — they run against mocks of the peer, and the defects that have actually shipped were disagreements between real implementations.
