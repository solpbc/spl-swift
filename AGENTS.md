...
- **Before tagging a release, confirm that every supported consumer platform has a recorded real-peer integration baseline and run this repository's CI for the candidate.** A fresh consumer run may supplement that baseline when the change warrants it. A regression in current evidence blocks the tag; a standing failure is recorded. A gate that did not run is not a pass. After tagging, validate the exact tag through each consumer's normal adoption gate before that consumer ships. Repository tests do not exercise the shipped consumer products on their supported platforms, so they do not replace real-peer evidence.
- **A tag is not the finish line.** A consumer still pinned to an older tag is not running this code. Advancing every consumer's pin is part of the change, not follow-up work.
