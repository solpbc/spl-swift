...
struct RaceCoordinatorTests {
    ...
    @Test func publicCandidateOrderingAgreesWithRaceCoordinatorSorted() {
        let relay = TransportEndpoint.relay(
            endpoint: URL(string: "wss://relay.example/session")!,
            instanceID: "instance",
            deviceToken: "token"
        ...
        #expect(
            CandidateOrdering.sorted(endpoints, preferredEndpoint: preferred)
                == RaceCoordinator<Int>.sorted(endpoints, preferredEndpoint: preferred)
        )
        #expect(
            CandidateOrdering.sorted(endpoints)
                == RaceCoordinator<Int>.sorted(endpoints)
        )
    }
